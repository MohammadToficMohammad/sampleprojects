...
public class MqClientQueueName {
	...
	/*
	@Autowired
	MqClientEventsToSubscribe mqClientEventsToSubscribe;
	
	
	public String[] buildFor(String name) {
    	
    	var list=mqClientEventsToSubscribe.EventsToSubscribe.stream().map(x->x+".mqevents").collect(Collectors.toList());
