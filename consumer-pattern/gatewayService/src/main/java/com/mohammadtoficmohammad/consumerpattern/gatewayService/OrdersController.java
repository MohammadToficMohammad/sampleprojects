...
import com.mohammadtoficmohammad.consumerpattern.MqEventsServerAbstracts.MqEventSender;
import com.mohammadtoficmohammad.consumerpattern.MqEventsServerAbstracts.MqEventSender.EventSenderDto;
...
public class OrdersController {
	...
	public String createOrder(@RequestBody OrderCreateDTO orderCreateDTO) {
		...
		if (businessManagerClient.checkAndSetLock()) {
			var event = new CreateOrderEvent<EventSenderDto>(EventSenderDto.class).build(orderCreateDTO.name,
			...
			// span.event("CreateOrderEvent: "+"orderName:"+orderCreateDTO.name+"
			// orderPrice:"+orderCreateDTO.price);
	...
	public void sendScheduledOrder() {
		System.out.println("Scheduled sending.. ");
		var order = new OrderCreateDTO();
		order.name = "ScheduledOrder";
		order.price = 1000;
